...
contract LibraGenesisRewardPool {
    ...
    // TESTNET
    uint256 public libraPerSecond = 3.0555555 ether; // 11000 LIBRA / (1h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "LibraGenesisPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "LibraGenesisPool: existing pool?");
        }
    }
